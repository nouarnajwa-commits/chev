...
app.post('/api/:pid/photos/:id/reclassify', async (req,res) => {
  const data=loadProject(req.params.pid), idx=data.photos.findIndex(p=>p.id===req.params.id);
  if (idx===-1) return res.status(404).json({error:'Not found'});
  ...
  try {
    ...
    Object.assign(data.photos[idx],cls,{ai_classified:true}); saveProject(req.params.pid,data); res.json(data.photos[idx]);
...
app.post('/api/:pid/vectorize-element', express.json(), async (req, res) => {
  const { photo_id, element, keyword, cache_key } = req.body || {};
...
If the element name contains a STANDARD REFERENCE (e.g. "IPE200", "HEA160", "500x200x200", "C25/30", "S235", "GL24h", "B60"), use the EXACT values from European technical standards (EN 1993 for steel, EN 1992 for concrete, EN 771 for masonry, EN 14080 for glulam).
...
Reply ONLY with JSON, all values realistic for the specific reference if standard, in metric units:
{
...
Use null for any property that doesn't apply.`;

  try {
    const r = await fetch('https://api.mammouth.ai/v1/chat/completions', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${key}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: 'gpt-4.1', max_tokens: 300, messages: [{ role: 'user', content: prompt }] })
    });
    if (!r.ok) {
